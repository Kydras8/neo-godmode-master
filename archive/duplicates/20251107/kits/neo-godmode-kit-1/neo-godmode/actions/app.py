...
def R():
    global _rr
    if _rr is None:
        _rr = redis.Redis(host=REDIS_HOST, port=REDIS_PORT, decode_responses=True)
    return _rr
...
@app.post("/enqueue")
def enqueue_job(inp: EnqueueIn, dep: bool = Depends(verify_api_key), creds: HTTPAuthorizationCredentials = Depends(bearer)):
    verify_jwt(required_scope="task_update", creds=creds)
    jid = str(uuid.uuid4())
    job = {"id": jid, "task": inp.task, "payload": inp.payload or {}}
    R().rpush(QUEUE, json.dumps(job))
    return {"queued": True, "id": jid}
